	...
	: vm(vmr), re_str(F_TAG_ARRAY, 0/*dmy*/), re_pos(F_TAG_ARRAY, 0/*dmy*/), re_len(F_TAG_ARRAY, 0/*dmy*/)
{
	...
	re_option   = std::regex::ECMAScript;
}
...
CValue	CSystemFunction::RE_SEARCH(const CValue &arg, yaya::string_t &d, int &l)
{
	...
	int	t_result;
	try {
		std::basic_regex<yaya::char_t> regex(arg1.c_str(),std::regex::ECMAScript | std::regex::collate | re_option);
		std::match_results<yaya::string_t::const_iterator>	result;
		t_result = (int)std::regex_search(arg0, result, regex);
		if (t_result)
...
CValue	CSystemFunction::RE_ASEARCH(const CValue &arg, yaya::string_t &d, int &l)
{
	...
	try {
		std::basic_regex<yaya::char_t> regex(key.GetValueString(),std::regex::ECMAScript | std::regex::collate | re_option);

		for(int i = 1; i < sz; i++) {
			try {
				std::match_results<yaya::string_t::const_iterator> result;
				int t_result = (int)std::regex_search(arg.array()[i].GetValueString(), result, regex);
				if (t_result) {
					//結果ためこんでもあまり意味ないので
					//StoreReResultDetails(result);
					return CValue(i-1);
...
CValue	CSystemFunction::RE_MATCH(const CValue &arg, yaya::string_t &d, int &l)
{
	...
	try {
		std::basic_regex<yaya::char_t> regex(arg1.c_str(),std::regex::ECMAScript | std::regex::collate | re_option);
		std::match_results<yaya::string_t::const_iterator>	result;
		t_result = (int)std::regex_match(arg0, result, regex);
		if (t_result)
			StoreReResultDetails(result);
	}
	catch(const std::regex_error &) {
...
CValue	CSystemFunction::RE_GREP(const CValue &arg, yaya::string_t &d, int &l)
{
	...
	try {
		...
		std::match_results<yaya::string_t::const_iterator>	result;
		for( ; ; ) {
			if (!std::regex_search(search_point, search_end, result, regex))
				break;
			t_result++;
...
/*		if ( opt.find(L"m") == yaya::string_t::npos ) {
			re_option |= std::regex::no_mod_m;
		}
		if ( opt.find(L"s") != yaya::string_t::npos ) {
			re_option |= std::regex::mod_s;
		}
		if ( opt.find(L"x") != yaya::string_t::npos ) {
			re_option |= std::regex::mod_x;
		}*/
		if ( opt.find(L"i") != yaya::string_t::npos ) {
			re_option |= std::regex::icase;
		}
	}

	yaya::string_t result = L"";

/*	if ( (re_option & std::regex::no_mod_m) == 0 ) {
		result += L"m";
	}
	if ( (re_option & std::regex::mod_s) != 0 ) {
		result += L"s";
	}
	if ( (re_option & std::regex::mod_x) != 0 ) {
		result += L"x";
	}*/
	if ( (re_option & std::regex::icase) != 0 ) {
		result += L"i";
	}
...
CValue	CSystemFunction::RE_SPLIT_CORE(const CValue &arg, yaya::string_t &d, int &l, const yaya::char_t *fncname, std::vector<yaya::string_t> *replace_array, size_t num)
{
	...
	try {
		...
		for( ; ; ) {
			...
			if ( replace_array ) {
				replace_array->push_back(result.format(arg.array()[2].GetValueString(),std::regex_constants::format_default));
			}
 ...
 * -----------------------------------------------------------------------
 */
void	CSystemFunction::StoreReResultDetails(std::match_results<yaya::string_t::const_iterator> &result)
{
	int	sz = result.size();
	for(int i = 0; i < sz; i++)
